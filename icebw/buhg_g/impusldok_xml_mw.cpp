/*$Id: impusldok_xml_mw.c,v 1.27 2014/05/07 10:58:06 sasa Exp $*/
/*11.07.2015	06.02.2015	Белых А.И.	impusldok_xml_mw.c
*/
#include "buhg_g.h"
...
class impusldok_xml_mw_data
 {
  ...
  impusldok_xml_mw_data() //Конструктор
   {
    kl_shift=0;
    voz=1;
   }
...
gboolean   impusldok_xml_mw_key_press(GtkWidget *widget,GdkEventKey *event,class impusldok_xml_mw_data *data);
void  impusldok_xml_mw_knopka(GtkWidget *widget,class impusldok_xml_mw_data *data);
void    impusldok_xml_mw_vvod(GtkWidget *widget,class impusldok_xml_mw_data *data);
int impmatdok_xml_zap(class impusldok_xml_mw_data *data);
void  impmatdok_xml_v_e_knopka(GtkWidget *widget,class impusldok_xml_mw_data *data);
...
int impusldok_xml_mw(class iceb_imp_xml_nn_r *rkv,GtkWidget *wpredok)
{
class iceb_u_str repl("");
class impusldok_xml_mw_data data;
char strsql[512];
data.rk=rkv;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(impusldok_xml_mw_key_press),&data);
...
g_signal_connect(data.entry[E_DATA], "activate",G_CALLBACK(impusldok_xml_mw_vvod),&data);
...
data.knopka_enter[E_SKLAD]=gtk_button_new_with_label(gettext("Подразделение"));
...
g_signal_connect(data.entry[E_SKLAD], "activate",G_CALLBACK(impusldok_xml_mw_vvod),&data);
...
g_signal_connect(data.entry[E_NOMDOK], "activate",G_CALLBACK(impusldok_xml_mw_vvod),&data);
...
g_signal_connect(data.entry[E_KOD_OP], "activate",G_CALLBACK(impusldok_xml_mw_vvod),&data);
...
data.knopka_enter[E_SHETU]=gtk_button_new_with_label(gettext("Счёт учёта"));
...
g_signal_connect(data.entry[E_SHETU], "activate",G_CALLBACK(impusldok_xml_mw_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(impusldok_xml_mw_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(impusldok_xml_mw_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(impusldok_xml_mw_knopka),&data);
...
switch (knop)
 {
  ...
  case E_SKLAD:
    ...
    if(l_uslpodr(1,&kod,&naim,data->window) == 0)
     gtk_entry_set_text(GTK_ENTRY(data->entry[E_SKLAD]),kod.ravno());
  ...
  case E_NOMDOK:
    ...
    iceb_nomnak(data->rk->datdok.ravno(),data->rk->sklad.ravno(),&data->rk->nomdok,1,1,0,data->window);
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_NOMDOK]),data->rk->nomdok.ravno());
  ...
  case E_KOD_OP:
    if(l_uslopp(1,&kod,&naim,data->window) == 0)
      gtk_entry_set_text(GTK_ENTRY(data->entry[E_KOD_OP]),kod.ravno());
...
switch (enter)
 {
  ...
  case E_SKLAD:
    ...
    sprintf(strsql,"select kod from Uslpodr where kod=%d",data->rk->sklad.ravno_atoi());
    if(iceb_sql_readkey(strsql,data->window) != 1)
     {
      iceb_menu_soob(gettext("Не найден код подразделения в справочнике!"),data->window);
      return;
     }    
...
sprintf(strsql,"select vido from Usloper1 where kod='%s'",data->rk->kod_op.ravno_filtr());
...
if(data->rk->shetu.getdlinna() <= 1)
 {
  iceb_menu_soob(gettext("Не введён счёт учёта!"),data->window);
  return(1);
 }
...
class iceb_lock_tables lokt("LOCK TABLES Usldokum WRITE,icebuser READ");

/*Проверяем есть ли такой номер документа*/
sprintf(strsql,"select nomd from Usldokum where datd >= '%04d-01-01' and datd <= '%d-12-31' and podr=%s and nomd='%s'",
...
sprintf(strsql,"insert into Usldokum (tp,datd,podr,kontr,nomd,nomnn,kodop,nds,ktoi,vrem,datpnn,pn,k00) values(%d,'%s',%d,'%s','%s','%s','%s',%d,%d,%ld,'%s',%.2f,'%s')",
1,
