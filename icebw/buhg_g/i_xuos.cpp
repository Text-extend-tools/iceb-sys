/*$Id:$*/
/*23.05.2018	17.10.2007	Белых А.И.	i_xuos.c
Учёт основных средств
*/
#define  DVERSIQ "06.12.2018"
#include        "buhg_g.h"
...
menu_str.plus(gettext("Расчёт амортизации"));//6
menu_str.plus(gettext("Инструменты"));//7

while( kom >= 0 )   
 {

  kom=iceb_menu_start(kom,version,DVERSIQ,gettext("Учёт основных средств"),fioop.ravno(),iceb_get_pnk("00",0,NULL),nom_op,&menu_str,"white");
  switch(kom)
   {
    case 0:
...
int nomer=0;
titl.plus(gettext("Настрока концовки документов"));

zagolovok.plus(gettext("Настрока концовки документов"));

punkt_m.plus(gettext("Ввод текста до подписей накладной"));//1
punkt_m.plus(gettext("Ввод текста после подписей накладной"));//0
...
switch (nomer)
 {
  ...
  case 0:
    iceb_f_redfil("uosnakend_dp.alx",0,NULL);
    return;

  case 1:
    iceb_f_redfil("uosnakend_pp.alx",0,NULL);
    return;
...
punkt_m.plus(gettext("Настройка концовки документов"));//15
...
punkt_m.plus(gettext("Распечатка ведомости аморт-отчислений для налогового учёта"));//0
punkt_m.plus(gettext("Распечатка ведомости аморт-отчислений для бух. учёта"));//1
...
punkt_m.plus(gettext("Расчёт амортизации для налогового учёта"));//0
punkt_m.plus(gettext("Расчёт амортизации для бухгалтерского учёта"));//1
...
punkt_m.plus(gettext("Распечатка ведомости аморт-отчислений"));//1
punkt_m.plus(gettext("Распечатать реестр проводок по видам операций"));//2
punkt_m.plus(gettext("Распечатать ведомость износа и остаточной стоимости"));//3
punkt_m.plus(gettext("Распечатать движение по документам"));//4
punkt_m.plus(gettext("Распечатать список лет за которые введена информация"));//5
punkt_m.plus(gettext("Распечатать инвентаризационные ведомости"));//6
punkt_m.plus(gettext("Распечатать ведомость остаточной стоимости"));//7
punkt_m.plus(gettext("Расчёт балансовой стоимости по мат.ответственным и счетам учёта"));//8
punkt_m.plus(gettext("Расчёт движения основных средств по счетам списания/получения"));//9
...
punkt_m.plus(gettext("Экспорт основных средств"));//2
