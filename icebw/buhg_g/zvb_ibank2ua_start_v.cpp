...
/*04.12.2010	16.10.2009	Белых А.И.	zvb_ibank2ua_v.c
...
class zvb_ibank2ua_v_data
 {
  ...
  class zvb_ibank2ua_rek *rk;
  
  zvb_ibank2ua_v_data() //Конструктор
   {
    kl_shift=0;
    voz=1;
   }
...
gboolean   zvb_ibank2ua_v_key_press(GtkWidget *widget,GdkEventKey *event,class zvb_ibank2ua_v_data *data);
void  zvb_ibank2ua_v_knopka(GtkWidget *widget,class zvb_ibank2ua_v_data *data);
void    zvb_ibank2ua_v_vvod(GtkWidget *widget,class zvb_ibank2ua_v_data *data);
int zvb_ibank2ua_zap(class zvb_ibank2ua_v_data *data);

void  zvb_ibank2ua_v_e_knopka(GtkWidget *widget,class zvb_ibank2ua_v_data *data);
...
class zvb_ibank2ua_v_data data;
char strsql[512];
iceb_u_str kikz;
...
sprintf(strsql,"%s %s",gettext("Перечисление на картсчета"),"iBank2 UA");
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(zvb_ibank2ua_v_key_press),&data);
...
g_signal_connect(data.knopka_enter[E_DATPP],"clicked",G_CALLBACK(zvb_ibank2ua_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATPP], "activate",G_CALLBACK(zvb_ibank2ua_v_vvod),&data);
...
g_signal_connect(data.entry[E_NOMERPP], "activate",G_CALLBACK(zvb_ibank2ua_v_vvod),&data);
...
g_signal_connect(data.entry[E_MESZP],"activate",G_CALLBACK(zvb_ibank2ua_v_vvod),&data);
...
g_signal_connect(data.entry[E_KOMENT], "activate",G_CALLBACK(zvb_ibank2ua_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(zvb_ibank2ua_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(zvb_ibank2ua_v_knopka),&data);
...
//printf("zvb_ibank2ua_v_key_press\n");
switch(event->keyval)
 {
...
int knop=atoi(gtk_widget_get_name(widget));
//g_print("zvb_ibank2ua_v_knopka knop=%d\n",knop);
...
//g_print("zvb_ibank2ua_v_vvod enter=%d\n",enter);
