/* $Id: avtpromuw.c,v 1.30 2013/09/26 09:46:45 sasa Exp $ */
/*10.07.2015    19.10.1994      Белых А.И.      avtpromuw.c
    ...
    if(mvz == 0)
     {
      ...
      zagolovok.plus(gettext("Выберите нужный счёт для НДС."));
  ...
  if(tpz == 2)
   {
    ...
    //Общая сумма по документу с учётом скидки
      ...
      if(fabs(bb) > 0.009 && shp2[0] != '\0' && variant == 1)      
       {
        ...
        iceb_zapmpr(gp.ravno(i),mp.ravno(i),dp.ravno(i),shp4,shp2,kontr4.ravno(),kontr2.ravno(),deb,kre,"",kolpr,kekv,&sp_prov,&sum_prov_dk,ff_prot);
       }
     }
      ...
      if(fabs(bb) > 0.009 && shp2[0] != '\0')      
       {
        ...
        iceb_zapmpr(gp.ravno(i),mp.ravno(i),dp.ravno(i),shp2,shp5,kontr2.ravno(),kontr5.ravno(),deb,kre,"",kolpr,kekv,&sp_prov,&sum_prov_dk,ff_prot);
       }
     }       
      ...
      if(fabs(bb) > 0.009 )      
       {
        ...
        iceb_zapmpr(gp.ravno(i),mp.ravno(i),dp.ravno(i),shp7,shp2,kontr7.ravno(),kontr2.ravno(),deb,kre,"",kolpr,kekv,&sp_prov,&sum_prov_dk,ff_prot);
       }
     }            
   }
 }
