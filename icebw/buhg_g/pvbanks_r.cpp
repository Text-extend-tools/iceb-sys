...
/*03.10.2016	08.04.2004	Белых А.И.	pvbanks_r.c
...
for(int snom=0 ; snom < kolkontr; snom++)
 {
  ...
  if(kod_banka == 7) /*Укрэксимбанк*/
    ukreksim_zstrw(fftmp,ffr,&nomer,sum,fio.ravno(),bankshet.ravno(),tabnom.ravno(),dt,mt,gt);

  if(kod_banka == 8) /*хресщатик*/
    zvb_hreshatik_strw(d_pp,m_pp,g_pp,&nomer,fio.ravno(),bankshet.ravno(),inn.ravno(),sum,fftmp,ffr);

  if(kod_banka == 9) /*ibank2ua*/
   zvb_ibank2ua_strw(&nomer,fio.ravno(),bankshet.ravno(),inn.ravno(),sum,fftmp,ffr);
...
if(metka_banka == 4)
 if(ukrsib_startw(imafsp,imaftmp,&ope_tor,&fftmp,&ffr,wpredok) != 0)
  return;

if(metka_banka == 11)
 if(zvb_ukrgaz_startw(imafsp,imafdis,imaftmp,&fftmp,&ffr,wpredok) != 0)
  return;

if(metka_banka == 13)
 if(zvb_kievrus_startw(imafsp,imafdis,imaftmp,&fftmp,&ffr,wpredok) != 0)
  return;
if(metka_banka == 14)
 if(zvb_corp2_startw(imafsp,imafdis,imaftmp,&fftmp,&ffr,wpredok) != 0)
  return;

if(metka_banka == 15)
 if(zvb_privatbank_startw(imafsp,imafdis,imaftmp,&branch,&zpkod,&rlkod,&fftmp,&ffr,wpredok) != 0)
  return;
...
for(int snom=0 ; snom < kolkontr; snom++)
 {
  ...
  sumai+=sum;
  if(metka_banka == 4)
    ukrsib_strw(dt,mt,gt,sum,bankshet.ravno(),fio.ravno(),&nomer,tabnom.ravno(),ope_tor.ravno(),ffr,fftmp);
  if(metka_banka == 11)
    zvb_ukrgaz_strw(++nomer,fio.ravno(),bankshet.ravno(),sum,inn.ravno(),tabnom.ravno(),fftmp,ffr);
  if(metka_banka == 13)
    zvb_kievrus_strw(++nomer,fio.ravno(),bankshet.ravno(),sum,inn.ravno(),tabnom.ravno(),fftmp,ffr);
  if(metka_banka == 14)
    zvb_corp2_strw(++nomer,fio.ravno(),bankshet.ravno(),sum,inn.ravno(),tabnom.ravno(),fftmp,ffr);
  if(metka_banka == 15)
    zvb_privatbank_strw(++nomer,fio.ravno(),bankshet.ravno(),sum,inn.ravno(),tabnom.ravno(),branch.ravno(),zpkod.ravno(),rlkod.ravno(),fftmp,ffr);
...
if(metka_banka == 13)
  zvb_kievrus_endw(imafdis,imaftmp,sumai,nomer,ffr,fftmp,wpredok);
if(metka_banka == 14)
  zvb_corp2_endw(imafdis,imaftmp,sumai,nomer,ffr,fftmp,wpredok);
if(metka_banka == 15)
  zvb_privatbank_endw(imafdis,imaftmp,sumai,nomer,ffr,fftmp,wpredok);
