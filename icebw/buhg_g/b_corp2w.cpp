...
/*20.05.2016	02.11.2014	Белых А.И. 	b_corp2w.c
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  fprintf(ff1," %-12.12s%-15.15s%-12.12s%-15.15s%1.1s%20d%3.3s%-10.10s%4.4s%04d%02d%02d%04d%02d%02d%-*.*s%-*.*s%-*.*s%-14.14s%-14.14s",
  rec.mfo.ravno(),
  rec.nsh.ravno(),
  rec.mfo1.ravno(),
  rec.nsh1.ravno(),
...
b_corp2_f(&f[shetshik++],"kk_a", 'C', 15, 0,&header_len,&rec_len);
b_corp2_f(&f[shetshik++],"kb_b", 'C', 12, 0,&header_len,&rec_len);
b_corp2_f(&f[shetshik++],"kk_b", 'C', 15, 0,&header_len,&rec_len);
