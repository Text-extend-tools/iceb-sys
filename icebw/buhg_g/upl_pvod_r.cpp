...
/*18.05.2018	30.03.2008	Белых А.И.	upl_pvod_r.c
Расчёт отчёта 
*/
#include <errno.h>
#include "buhg_g.h"
...
//  printw("%s\n",strsql);
  ...
  if(nomer != 0)
   ...
   uplrpl_r_nad(dn,mn,gn,dk,mk,gk,kodvd.ravno(),naimvod.ravno(),kodav.ravno(),naimavt.ravno(),nom_znak.ravno(),ff,data->window);
    ...
    if(dp != 0)
     {
      ...
      iperedano+=uplrpl_r_per_top(dp,mp,gp,dd,md,gd,kodvd.ravno(),kodav.ravno(),nom_zap,0,data->window);
     }
    nom_zap=atoi(row[5]);
    dp=dd;
    mp=md;
...
---------------------------------------------------------------------------------------------------------------------------------------\n");
    ...
    iperedano+=uplrpl_r_per_top(dp,mp,gp,dk,mk,gk,kodvd.ravno(),kodav.ravno(),nom_zap,1,data->window);
