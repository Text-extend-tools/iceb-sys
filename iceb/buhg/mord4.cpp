/*$Id: mord4.c,v 5.20 2013/09/26 09:43:35 sasa Exp $*/
/*15.07.2015	04.09.2007	Белых А.И.	mord4.c
Расчёт и распечатка мемориальных ордеров 4
Записи оплаты по документам показываются в кредитовой колонке
Проводки к документам в дебетовой колонке
*/
#include <math.h>
#include <errno.h>
#include "buhg.h"
...
int mord4(int nom_ord,class spis_oth *oth)
{
...
if(nom_ord == 4)
  VV.VVOD_SPISOK_add_ZAG(gettext("Расчёт мемориального ордера N4")); /*Оплата по дебету*/
...
if(nom_ord == 4)
 fprintf(ff,"%32sТипова форма N 408-авт (бюджет)\n","");

iceb_t_insfil("mord_start.alx",ff);

fprintf(ff,"\n%15s Меморіальний ордер N%d\n","",nom_ord);

fprintf(ff,"%5s за період з %02d.%02d.%d р. по %02d.%02d.%d р.\n","",dn,mn,gn,dk,mk,gk);

if(nom_ord == 4)
  fprintf(ff,"  Накопичувальна відомість за розрахунками з іншими дебіторами\n");
...
class SPISOK iceb_sheta_prov1_deb; 
class masiv_din_double iceb_sheta_prov1_deb_sum; /*Суммы*/
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  sprintf(strsql,"select datp,sh,shk,deb from Prov where kto='%s' and pod=%s and nomd='%s' and datd='%s'\
and kekv=%s and deb <> 0. order by datp asc",row[0],row[1],row[2],row[3],row[4]);
    ...
    if(SRAV(shet_or.ravno(),row1[1],1) == 0 )
     {
      ...
      if(sravmydat(d,m,g,dn,mn,gn) >= 0 && sravmydat(d,m,g,dk,mk,gk) <= 0)
       {
        fprintf(ff_prot,"Период suma=%f\n",suma);
        obor_deb+=suma;

        prov1.plus(strsql);

        if((nomer_vsp=iceb_sheta_prov1_deb.find(row1[2])) < 0)
         iceb_sheta_prov1_deb.plus(row1[2]);

        iceb_sheta_prov1_deb_sum.plus(suma,nomer_vsp);
       }
     }
    else
     {
  ...
  if(metka_ps == 3) /*в учёте основных средств нумерация документов не зависит от подразделения*/
  ...
  for(int ii=1;;ii++)
   {
    ...
    if(nomer_deb < kolih_deb)
     {
      polen(oplata.ravno(nomer_deb),data[1],sizeof(data),1,'|');
      polen(oplata.ravno(nomer_deb),kredit_shet[1],sizeof(kredit_shet[1]),2,'|');
      polen(oplata.ravno(nomer_deb),&sum_prov[1],3,'|');
      polen(oplata.ravno(nomer_deb),nomdokum[1],sizeof(nomdokum[1]),4,'|');
      strncpy(debet_shet[1],shet_or.ravno(),sizeof(debet_shet[1])-1);          
      nomer_deb++;
     }

    if(nomer_kre < kolih_kre)
     {
      polen(prov1.ravno(nomer_kre),data[0],sizeof(data),1,'|');
      polen(prov1.ravno(nomer_kre),kredit_shet[0],sizeof(kredit_shet[0]),3,'|');
      polen(prov1.ravno(nomer_kre),debet_shet[0],sizeof(kredit_shet[0]),2,'|');
      polen(prov1.ravno(nomer_kre),&sum_prov[0],4,'|');

      strncpy(nomdokum[0],row[2],sizeof(nomdokum[0])-1);
      nomer_kre++;
      metka_kre++;
     }
    ...
    if(metka_kre == 0 && nomer_kre == kolih_kre && nomer_dr < kolih_dr)
     {
      ...
      if(metka_rsdr > 1)
       {        
        polen(prov2.ravno(nomer_dr),data[0],sizeof(data),1,'|');
        polen(prov2.ravno(nomer_dr),kredit_shet[0],sizeof(kredit_shet[0]),3,'|');
        polen(prov2.ravno(nomer_dr),debet_shet[0],sizeof(debet_shet[0]),2,'|');
        polen(prov2.ravno(nomer_dr),&sum_prov[0],4,'|');
    ...
    memset(suma_char,'\0',sizeof(suma_char));
    if(sum_prov[0] != 0.)
     sprintf(suma_char,"%.2f",sum_prov[0]);
    ...
    memset(suma_char,'\0',sizeof(suma_char));
    if(sum_prov[1] != 0.)
     sprintf(suma_char,"%.2f",sum_prov[1]);
...
fprintf(ff,"\nУсього до кредіту субрахунків %s: Субсчёт     Сумма\n",shet_or.ravno());
double itogo=0.;                               
for(int ii=0; ii < i_sheta_oplati.kolih(); ii++)
 {
  fprintf(ff,"%34s %-10s %10.2f\n","",i_sheta_oplati.ravno(ii),i_sheta_oplati_sum.ravno(ii));
...
itogo=0.;
for(int ii=0; ii < iceb_sheta_prov1_deb.kolih(); ii++)
 {
  fprintf(ff,"%34s %-*s %10.2f\n",
  "",
  iceb_tu_kolbait(10,iceb_sheta_prov1_deb.ravno(ii)),iceb_sheta_prov1_deb.ravno(ii),
  iceb_sheta_prov1_deb_sum.ravno(ii));
  
  itogo+=iceb_sheta_prov1_deb_sum.ravno(ii);
...
fprintf(ff,"\nДругі записи: по кредіту субрахунків: Субсчёт     Сумма\n");
itogo=0.;
