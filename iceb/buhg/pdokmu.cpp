/*$Id: pdokmu.c,v 5.7 2013/12/31 11:48:10 sasa Exp $*/
/*15.07.2015	23.12.2009	Белых А.И.	pdokmu.c
...
fprintf(ff,"%s:%s\n",gettext("Записал"),iceb_t_kszap(row[3]));
fprintf(ff,"%s:%s\n",gettext("Дата записи"),iceb_tu_vremzap(row[4]));

if(row[1][0] != '\0')
  fprintf(ff,"%s:%s\n",gettext("Номер налоговой накладной"),row[1]);
if(row[5][0] != '0')
  fprintf(ff,"%s:%s\n",gettext("Дата получения налоговой накладной"),iceb_tu_datzap(row[5]));
if(atoi(row[7]) == 0)
  ...
  switch (nomerz)
   {
    case 1:
     polen(row[1],&bros,1,'#');
     fprintf(ff,"%s:%s ",gettext("Доверенность"),bros.ravno());
     polen(row[1],&bros,2,'#');
     fprintf(ff,"%s:%s\n",gettext("Дата"),bros.ravno());
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  if(iceb_tu_strlen(naim_mat) > 30)
   fprintf(ff,"%6s %s\n","",iceb_tu_adrsimv(30,naim_mat));
 }
...
fprintf(ff,"\
-------------------------------------------------------------------------------\n");
fprintf(ff,"%*s:%10.2f\n",iceb_tu_kolbait(67,gettext("Итого")),gettext("Итого"),itogo);

if(suma_kor < 0.)
  fprintf(ff,"%*s:%10.2f\n",iceb_tu_kolbait(67,gettext("Скидка")),gettext("Скидка"),suma_kor);
if(suma_kor > 0.)
  fprintf(ff,"%*s:%10.2f\n",iceb_tu_kolbait(67,gettext("Надбавка")),gettext("Надбавка"),suma_kor);
...
if(mnds == 0) 
 {
  ...
  oplata=oplata+nds;
  fprintf(ff,"%*s:%10.2f\n",iceb_tu_kolbait(67,gettext("НДС")),gettext("НДС"),nds);
 }    
if(itogo != oplata)
 fprintf(ff,"%*s:%10.2f\n",iceb_tu_kolbait(67,gettext("Итого")),gettext("Итого"),oplata);
podpis(ff);
fclose(ff);
