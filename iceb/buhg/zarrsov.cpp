/*$Id:$*/
/*03.10.2015	03.10.2015	Белых А.И.	zarrsov.c
Распечатка свода отработанного времени
*/
#include <math.h>
#include "buhg.h"

void zarrsov(short mn,short gn,
short mk,short gk,
int tabnom,
class iceb_zarrsov *svod_otvr,FILE *ff)
{
char strsql[1024];
SQL_str row;
class SQLCURSOR cur;
...
int kolmes=period(1,mn,gn,31,mk,gk,1);
...
for(int nom=0; nom < svod_otvr->kod.kolih(); nom++)
 {
  ...
  fprintf(ff,"%-*.*s|",
  iceb_tu_kolbait(14,strsql),
  iceb_tu_kolbait(14,strsql),
  strsql);

  if(max_dl_str < iceb_tu_strlen(strsql))
   max_dl_str=iceb_tu_strlen(strsql);
 }

fprintf(ff,"%-*.*s|\n",
iceb_tu_kolbait(14,gettext("Итого")),
iceb_tu_kolbait(14,gettext("Итого")),
gettext("Итого"));
  ...
  for(int nom=0; nom < svod_otvr->kod.kolih(); nom++)
   {
    ...
    fprintf(ff,"%-*.*s|",
    iceb_tu_kolbait(14,iceb_tu_adrsimv((nom_str+1)*14,strsql)),
    iceb_tu_kolbait(14,iceb_tu_adrsimv((nom_str+1)*14,strsql)),
    iceb_tu_adrsimv((nom_str+1)*14,strsql));
   }  
  fprintf(ff,"%14s|\n","");
 }
//fprintf(ff,"%14s|\n","");
...
for(int nom=0; nom <= svod_otvr->kod.kolih(); nom++)
 {
  fprintf(ff,"%-*.*s|%-*.*s|%-*.*s|",
  iceb_tu_kolbait(4,gettext("Рабочие")),
  iceb_tu_kolbait(4,gettext("Рабочие")),
  gettext("Рабочие"),
  iceb_tu_kolbait(4,gettext("Календарные")),
  iceb_tu_kolbait(4,gettext("Календарные")),
  gettext("Календарные"),
  iceb_tu_kolbait(4,gettext("Часы")),
  iceb_tu_kolbait(4,gettext("Часы")),
  gettext("Часы"));
 }
fprintf(ff,"\n");
