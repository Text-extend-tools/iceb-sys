...
/*29.05.2016    06.01.1993      Белых А.И.      dolhnn.c
Расчёт начислений
*/
#include        <ctype.h>
#include        <errno.h>
...
double dolhnn(int tn, //Табельный номер
...
sprintf(strsql,"%s-%6s %s %3s %10s %s %5s %5s %6s %s\n",__FUNCTION__,row[0],iceb_tu_datzap_mg(row[1]),row[2],row[3],row[4],row[5],row[6],row[7],row[8]);
zar_pr_ins(strsql);
...
if(proverka(nah_only,kodokl,0,0) != 0)
 {
...
if(metka_sp == 5 || metka_sp == 6)
  oklad=okladtr(mp,gp,(int)oklad); /*oklad содержит тарифный разряд*/
...
zapzarp(&zp,nah,d,mp,gp,0,shet.ravno(),"",0,kp,"");
