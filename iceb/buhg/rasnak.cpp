/* $Id:$ */
/*27.04.2018    21.04.2018      Белых А.И.      rasnak.c
...
int rasnak(short dg,short mg,short gg, //дата документа
int skl,  //Склад
...
fprintf(f1,"%-*.*s ",iceb_tu_kolbait(60,nmo.ravno()),iceb_tu_kolbait(60,nmo.ravno()),nmo.ravno());
fprintf(f1,"\x1B\x45N%s %-.*s\x1B\x46\n",
kpos.ravno(),
iceb_tu_kolbait(60,nmo1.ravno()),nmo1.ravno());
...
*/
  ...
  fprintf(f1,"%3d %-*s %-*.*s %-*.*s %10.10g %11s",
  i,
  iceb_tu_kolbait(18+simv_plus,nomn),nomn,
  iceb_tu_kolbait(46,naim.ravno()),iceb_tu_kolbait(46,naim.ravno()),naim.ravno(),
...
if(mnt != 0)
   rasnak_vztr(dd,md,gd,skl,nomdok,&sumt,simv_plus,ff);
      
double itogo1=(itogo+sumkor)+bb+sumt;
strcpy(strsql,gettext("Итого"));

fprintf(ff,"%*s: %10.2f\n",iceb_tu_kolbait(dlinna,strsql),strsql,itogo1);
return(itogo1);
}
